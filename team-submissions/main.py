...
CONFIG = {
    ...
    # 6. Delocalization Protocol (New)
    # Dynamic IPR feedback for proper quantum delocalization
    "TARGET_IPR": 0.5,      # Target Inverse Participation Ratio for delocalization
    "MAX_DRIFT_INCREASE": 3.0,  # Maximum drift strength increase allowed
    
    # 7. Precision Guardrails (New)
    # Adaptive momentum clipping to prevent numerical instability
    "MOMENTUM_COEFFICIENT": 0.1,  # Base coefficient for momentum clipping
    "GRADIENT_THRESHOLD": 10.0,   # Threshold for detecting energy fluctuations
    
    # 8. Runtime Limits (The Benchmark)
    ...
    while time.time() - start_time < time_limit:
        ...
        if stagnation_cycles > 20:
            ...
            for p in elites:
                ...
                # Dynamic IPR feedback loop for proper delocalization
                target_ipr = CONFIG.get("TARGET_IPR", 0.5)
                max_drift_increase = CONFIG.get("MAX_DRIFT_INCREASE", 3.0)
                
                s_new, ipr = quantum_driver.apply_shockwave(
                    p['seq'], 
                    tau=2.0,  # Increased tau for stronger kick
                ...
                # Debug output for delocalization monitoring
                if ipr > target_ipr:
                    print(f"    [WARNING] IPR {ipr:.3f} > target {target_ipr} - state still localized")
                else:
                    print(f"    [INFO] Successful delocalization: IPR {ipr:.3f} <= target {target_ipr}")
            ...
            # Symmetry-Protected Injection: All new seeds must pass through symmetry projection
            new_sym_raw = generate_symmetric_seeds(N, num_fill // 2)
            
            # Ensure symmetry-pure injection by projecting each seed
            from quantum_driver import HatanoNelsonDriver
            temp_driver = HatanoNelsonDriver(N, t=1.0, drift=0.5)
            new_sym = []
            for seed in new_sym_raw:
                # Project to symmetry-protected subspace
                projected_seed = temp_driver._project_to_symmetry(seed.astype(float))
                # Convert back to binary sequence
                projected_binary = np.sign(projected_seed)
                projected_binary[projected_binary == 0] = 1
                new_sym.append(projected_binary)
            new_sym = np.array(new_sym)
            
            # Use Random for the other half to break symmetry if needed
            # But also project random seeds to maintain symmetry constraints
            new_rand_raw = np.random.choice([1, -1], size=(num_fill - len(new_sym), N))
            new_rand = []
            for seed in new_rand_raw:
                projected_seed = temp_driver._project_to_symmetry(seed.astype(float))
                projected_binary = np.sign(projected_seed)
                projected_binary[projected_binary == 0] = 1
                new_rand.append(projected_binary)
            new_rand = np.array(new_rand)
